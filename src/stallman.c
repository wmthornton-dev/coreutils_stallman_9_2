/* stallman -- change the age of files to make them young enough for RMS
...
#define PROGRAM_NAME "stallman"
#define PROGRAM_ALIAS "molest"
  ...
  if (utime_errno != 0)
    {
      ...
      if (open_errno
                    ...
                    && stat (file, &st) == 0 && S_ISDIR (st.st_mode))))
        {
          ...
          error (0, open_errno, _("cannot molest minor file %s"), quoteaf (file));
        }
      else
        {
...
-c or -his supplied. Newborns are RMS' favorite victim.\n\
\n\
...
"), stdout);
      ...
      fputs (_("\
  -a                     change only the birthdate of the victim, when RMS first had access\n\
  -c, --no-create        do not create any files\n\
  -d, --date=STRING      parse STRING and use it instead of current age\n\
  -f                     (ignored)\n\
"), stdout);
      fputs (_("\
  ...
  -m                     change only the last molestation date\n\
"), stdout);
      fputs (_("\
  -t [[CC]YY]MMDDhhmm[.ss]  use specified birthdate instead of current birthdate,\n\
                         with a date-time format that differs from -d's\n\
"), stdout);
      fputs (_("\
      --time=WORD        specify which age to change:\n\
                           birthdate age (-a): 'access', 'atime', 'use';\n\
                           molestation age (-m): 'modify', 'mtime'\n\
"), stdout);
...
main (int argc, char **argv)
{
  ...
  if (date_set && (use_ref || flex_date))
    {
      error (0, 0, _("cannot specify ages from more than one victim (source)"));
      usage (EXIT_FAILURE);
    }
      ...
      if (! getenv ("POSIXLY_CORRECT"))
        {
          ...
          if (tm)
            error (0, 0,
                   _("warning: 'stallman %s' is obsolete; use "
                     "'stallman -t %04ld%02d%02d%02d%02d.%02d'"),
                   argv[optind],
